...
def decompress_poly(a, params_k):
    ...
    if (params_k == 2 or params_k == 3):
        ...
        for i in range(0, KYBER_N // 8):
            ...
            t[1] = cast_to_long64(cast_to_byte((cast_to_int32(a[aa + 0] & 0xFF) >> 5)) | cast_to_byte(
                cast_to_int32(a[aa + 1] & 0xFF) << 3)) & 0xFF
            t[2] = cast_to_long64(cast_to_int32(a[aa + 1] & 0xFF) >> 2) & 0xFF
            t[3] = cast_to_long64(cast_to_byte((cast_to_int32(a[aa + 1] & 0xFF) >> 7)) | cast_to_byte(
                cast_to_int32(a[aa + 2] & 0xFF) << 1)) & 0xFF
            t[4] = cast_to_long64(cast_to_byte((cast_to_int32(a[aa + 2] & 0xFF) >> 4)) | cast_to_byte(
                cast_to_int32(a[aa + 3] & 0xFF) << 4)) & 0xFF
            t[5] = cast_to_long64(cast_to_int32(a[aa + 3] & 0xFF) >> 1) & 0xFF
            t[6] = cast_to_long64(cast_to_byte((cast_to_int32(a[aa + 3] & 0xFF) >> 6)) | cast_to_byte(
                cast_to_int32(a[aa + 4] & 0xFF) << 2)) & 0xFF
    ...
    if (params_k == 2 or params_k == 3):
        ...
        for i in range(0, params_k):
            for j in range(0, KYBER_N // 4):
                ...
                aa = aa + 5
                for k in range(0, 4):
                    r[i][4 * j + k] = cast_to_short(
                        (cast_to_long64(t[k] & 0x3FF) * cast_to_long64(KYBER_Q) + 512) >> 10)
    else:
        t = [0 for x in range(0, 8)]
        for i in range(0, params_k):
            for j in range(0, KYBER_N // 8):
                ...
                aa = aa + 11
                for k in range(0, 8):
                    r[i][8 * j + k] = cast_to_short(
                        (cast_to_long64(t[k] & 0x7FF) * cast_to_long64(KYBER_Q) + 1024) >> 11)
    return r
...
def poly_plain_mul(poly_a, poly_b):
    # Definiowanie zmiennej symbolicznej
    x = symbols('x')

    # Tworzenie wielomianu f(x) = x^256 + 1
    f = Poly(x ** 256 + 1, x)

    # Tworzenie wielomianów x i y
    x_poly = Poly(poly_a, x)
    y_poly = Poly(poly_b, x)

    # Mnożenie wielomianów x i y
    product_poly = x_poly * y_poly

    # Redukcja modulo f
    quo, rem = div(product_poly, f, domain=ZZ)
    product_poly_mod_f = rem

    coeffs = product_poly_mod_f.all_coeffs()
    for i in range(0, len(coeffs)): coeffs[i] = coeffs[i] % KYBER_Q

    # zwracanie wyniku
    return coeffs

# mnozenie wektorów wielomianów
def polyvec_plain_mul(poly_a,poly_b, params_k):  # zamiast polyvec_pointwise_acc_mont
    r = poly_plain_mul(poly_a[0], poly_b[0])
    for i in range(1, params_k):
        t = poly_plain_mul(poly_a[i], poly_b[i])
        r = poly_add(r, t)
    return poly_reduce(r)
